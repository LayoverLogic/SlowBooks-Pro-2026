...
def _post_journal_credit(db_session, *, income_account_id, class_id,
                         amount, when, contra_account_id):
    ...
    db_session.add_all([
        TransactionLine(
            ...
            credit=Decimal("0"),
            home_currency_debit=Decimal(str(amount)),
            home_currency_credit=Decimal("0"),
        ),
        TransactionLine(
            transaction_id=txn.id,
            ...
            home_currency_debit=Decimal("0"),
            home_currency_credit=Decimal(str(amount)),
        ),
    ])
    db_session.commit()
    return txn
...
def test_monthly_revenue_picks_up_journal_credits_to_income(
    client, db_session, seed_accounts, uncategorized_class,
):
...
def test_monthly_revenue_excludes_credits_to_non_income_accounts(
    ...
    from app.models.accounts import AccountType
    asset = next(a for a in seed_accounts.values()
    ...
    liability = next(a for a in seed_accounts.values()
